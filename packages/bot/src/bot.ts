...
// Animated verdict stickers (.tgs = gzipped lottie, generated by
// scripts/gen-anims.mjs). Best-effort - a bad sticker never blocks a verdict.
async function sendVerdictSticker(
  ctx: { replyWithSticker: (f: InputFile) => Promise<unknown> },
  state: Verdict['state'],
): Promise<void> {
  try {
    await ctx.replyWithSticker(
      new InputFile(fileURLToPath(new URL(`../assets/${state}.tgs`, import.meta.url))),
    );
  } catch {
    /* sticker optional */
  }
...
bot.on(['message:photo', 'message:video', 'message:document', 'message:animation'], async (ctx) => {
  ...
  try {
    ...
    await sendVerdictSticker(ctx, verdict.state);
  } catch (e) {
